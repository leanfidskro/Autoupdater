...
namespace AutoUpdaterDotNET
{
    ...
    public static class AutoUpdater
    {
        ...
        internal static ILogger _lgr = LogManager.GetCurrentClassLogger();
        ...
        {
            _lgr.Trace("Autoupdater start called");
            try
            {
            ...
            if (!Running && _remindLaterTimer == null)
            {
                ...
                if (Synchronous)
                {
                    _lgr.Trace("Autoupdater running synchronous");
                    try
                    {
        ...
        private static object CheckUpdate(Assembly mainAssembly)
        {
            ...
            _lgr.Trace($"Installed current version {args.CurrentVersion} {args.InstalledVersion} detected IsUpdateAvailable {args.IsUpdateAvailable}");
            if (!Mandatory)
            {
                ...
                if (result is UpdateInfoEventArgs args)
                {
                    ...
                    else
                    {
                        if (args.IsUpdateAvailable)
                        {
                            ...
                            {
                                _lgr.Info("Update Available - Mandatory update and updatemode is ForcedDownload");
                                DownloadUpdate(args);
                                Exit();
                            }
                            else
                            {
                                _lgr.Info("Update Available - Showing update form");
